...
class TrackingService {
  ...
  Future<void> initialize() async {
    try {
      ...
      /// ‼️ **تبليغٌ متأخّر لما التُقط قبل الجهوزيّة.**
      ///
      /// الرابط العميق يصل **عند الإقلاع البارد** — أي قبل أن تكتمل هذه
      /// الدالّة. فالالتقاط يقع ويُحفَظ على القرص، لكنّ التبليغ كان يُتخطّى
      /// لغياب `appInstanceId` **فلا يصل الخادم أبداً**: وهي بالضبط حالة كلّ
      /// نقرةٍ على إعلان. (رُصد بالتشغيل 2026-08-13.)
      unawaited(_relayClickIds());
  ...
  Future<void> captureClickIds(Uri uri) async {
    ...
    // ‼️ يُنتظَر لا يُخمَّن: الرابط قد يسبق جهوزيّة المنظومة بثوانٍ.
    await ready;
